...
-- FIXED VERSION: Compatible with existing job_execution_log table
...
CREATE INDEX IF NOT EXISTS idx_retry_queue_next_retry ON job_retry_queue(next_retry_at) WHERE next_retry_at IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_retry_queue_job_user ON job_retry_queue(job_name, user_id);
CREATE INDEX IF NOT EXISTS idx_retry_queue_circuit_state ON job_retry_queue(circuit_breaker_state) WHERE circuit_breaker_state != 'closed';
CREATE INDEX IF NOT EXISTS idx_retry_queue_created ON job_retry_queue(created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_dlq_unresolved ON job_dead_letter_queue(created_at DESC) WHERE resolved = FALSE;
CREATE INDEX IF NOT EXISTS idx_dlq_job_name ON job_dead_letter_queue(job_name, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_dlq_user ON job_dead_letter_queue(user_id) WHERE user_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_dlq_failure_reason ON job_dead_letter_queue(failure_reason);
...
CREATE INDEX IF NOT EXISTS idx_circuit_breaker_active ON circuit_breaker_state(state) WHERE state != 'closed';
CREATE INDEX IF NOT EXISTS idx_circuit_breaker_timeout ON circuit_breaker_state(opened_at) WHERE state = 'open';
...
CREATE INDEX IF NOT EXISTS idx_retry_metrics_job_date ON job_retry_metrics(job_name, date DESC);
CREATE INDEX IF NOT EXISTS idx_retry_metrics_hourly ON job_retry_metrics(date, hour);
CREATE INDEX IF NOT EXISTS idx_retry_metrics_user ON job_retry_metrics(user_id) WHERE user_id IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_alert_history_recent ON job_alert_history(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_alert_history_unacked ON job_alert_history(created_at DESC) WHERE acknowledged = FALSE;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_job_retry_queue_updated_at') THEN
        CREATE TRIGGER update_job_retry_queue_updated_at BEFORE UPDATE ON job_retry_queue
            FOR EACH ROW EXECUTE FUNCTION update_updated_at();
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_circuit_breaker_state_updated_at') THEN
        CREATE TRIGGER update_circuit_breaker_state_updated_at BEFORE UPDATE ON circuit_breaker_state
            FOR EACH ROW EXECUTE FUNCTION update_updated_at();
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_job_retry_metrics_updated_at') THEN
        CREATE TRIGGER update_job_retry_metrics_updated_at BEFORE UPDATE ON job_retry_metrics
            FOR EACH ROW EXECUTE FUNCTION update_updated_at();
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_job_alert_config_updated_at') THEN
...
-- Monitoring views (FIXED to use started_at instead of executed_at)
...
SELECT 
    ...
    MAX(jel.started_at) as last_execution  -- FIXED: Using started_at
...
WHERE jel.started_at > NOW() - INTERVAL '24 hours'  -- FIXED: Using started_at
...
INSERT INTO job_alert_config (alert_name, alert_type, threshold_value, enabled) VALUES
    ('High Failure Rate', 'failure_rate', 20, true),
    ('Critical Failure Rate', 'failure_rate', 50, true),
    ('DLQ Growing', 'dlq_size', 50, true),
    ('DLQ Critical', 'dlq_size', 100, true),
    ('Circuit Breaker Open', 'circuit_breaker', 1, true),
    ('High Retry Rate', 'retry_rate', 30, true)
