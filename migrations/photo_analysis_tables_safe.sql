-- Photo Analysis Tables Migration (Safe Version)
...
CREATE INDEX IF NOT EXISTS idx_photo_sessions_deleted_at ON photo_sessions(deleted_at) WHERE deleted_at IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_photo_uploads_deleted_at ON photo_uploads(deleted_at) WHERE deleted_at IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_photo_tracking_suggestions_session_id ON photo_tracking_suggestions(session_id);
CREATE INDEX IF NOT EXISTS idx_photo_tracking_suggestions_analysis_id ON photo_tracking_suggestions(analysis_id);
...
CREATE INDEX IF NOT EXISTS idx_photo_comparisons_session_id ON photo_comparisons(session_id);
CREATE INDEX IF NOT EXISTS idx_photo_comparisons_created_at ON photo_comparisons(created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_photo_tracking_configurations_user_id ON photo_tracking_configurations(user_id);
CREATE INDEX IF NOT EXISTS idx_photo_tracking_configurations_session_id ON photo_tracking_configurations(session_id);
...
CREATE INDEX IF NOT EXISTS idx_photo_tracking_data_configuration_id ON photo_tracking_data(configuration_id);
CREATE INDEX IF NOT EXISTS idx_photo_tracking_data_recorded_at ON photo_tracking_data(recorded_at DESC);

-- RLS (Row Level Security) Policies
-- Note: We need to check if RLS is already enabled before enabling it

-- Enable RLS on all tables (with existence check)
DO $$ 
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_tables 
        WHERE tablename = 'photo_analyses' 
        AND rowsecurity = true
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_tables 
        WHERE tablename = 'photo_tracking_suggestions' 
        AND rowsecurity = true
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_tables 
        WHERE tablename = 'photo_comparisons' 
        AND rowsecurity = true
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_tables 
        WHERE tablename = 'photo_tracking_configurations' 
        AND rowsecurity = true
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_tables 
        WHERE tablename = 'photo_tracking_data' 
        AND rowsecurity = true
    ) THEN
...
DROP POLICY IF EXISTS "Users can view their own photo sessions" ON photo_sessions;
DROP POLICY IF EXISTS "Users can create their own photo sessions" ON photo_sessions;
DROP POLICY IF EXISTS "Users can update their own photo sessions" ON photo_sessions;
...
-- Drop and recreate Photo Uploads Policies
DROP POLICY IF EXISTS "Users can view photo uploads from their sessions" ON photo_uploads;
DROP POLICY IF EXISTS "Users can upload photos to their sessions" ON photo_uploads;
...
-- Drop and recreate Photo Analyses Policies
DROP POLICY IF EXISTS "Users can view analyses for their sessions" ON photo_analyses;
DROP POLICY IF EXISTS "Users can create analyses for their sessions" ON photo_analyses;
...
-- Drop and recreate Photo Tracking Suggestions Policies
DROP POLICY IF EXISTS "Users can view tracking suggestions for their sessions" ON photo_tracking_suggestions;
...
-- Drop and recreate Photo Comparisons Policies
DROP POLICY IF EXISTS "Users can view comparisons for their sessions" ON photo_comparisons;
...
-- Drop and recreate Photo Tracking Configurations Policies
DROP POLICY IF EXISTS "Users can view their own tracking configs" ON photo_tracking_configurations;
DROP POLICY IF EXISTS "Users can create their own tracking configs" ON photo_tracking_configurations;
DROP POLICY IF EXISTS "Users can update their own tracking configs" ON photo_tracking_configurations;
...
-- Drop and recreate Photo Tracking Data Policies
DROP POLICY IF EXISTS "Users can view tracking data for their configs" ON photo_tracking_data;
DROP POLICY IF EXISTS "Users can add tracking data to their configs" ON photo_tracking_data;
...
DO $$ 
BEGIN 
    RAISE NOTICE 'Photo analysis tables migration completed successfully!';
END $$;