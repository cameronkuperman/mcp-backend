-- SUPABASE MIGRATION FOR MISSING INTELLIGENCE FEATURES
-- Run this AFTER checking existing tables
-- Created: 2025-08-20
...
-- OR update user_ai_preferences with new columns
-- ============================================

-- Option A: Create new user_preferences table
...
-- Option B: If you prefer to use existing user_ai_preferences, run these instead:
-- ALTER TABLE user_ai_preferences ADD COLUMN IF NOT EXISTS
--     weekly_brief_enabled BOOLEAN DEFAULT true;
-- ALTER TABLE user_ai_preferences ADD COLUMN IF NOT EXISTS
--     weekly_brief_frequency VARCHAR(20) DEFAULT 'weekly';
-- ALTER TABLE user_ai_preferences ADD COLUMN IF NOT EXISTS
...
-- Only if you created user_preferences:
...
-- User Preferences Policies (if you created the table)
...
-- Job Execution Log Policies (admin only)
...
-- AND table_name IN ('weekly_health_briefs', 'job_execution_log', 'intelligence_cache', 'anonymous_symptom_patterns', 'user_preferences');

-- Check RLS is enabled:
-- SELECT tablename, rowsecurity 
-- FROM pg_tables 
-- WHERE schemaname = 'public' 
-- AND tablename IN ('weekly_health_briefs', 'intelligence_cache', 'job_execution_log');
