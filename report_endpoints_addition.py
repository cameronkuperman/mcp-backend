#!/usr/bin/env python3
"""
Additional Report Endpoints for run_oracle.py
Add these endpoints after the existing report endpoints
"""
...
# Helper Functions (add these to run_oracle.py)
...
async def extract_cardiac_patterns(data: dict) -> str:
    ...
    for scan in data.get("quick_scans", []):
        if any(symptom in str(scan.get("form_data", {})).lower() for symptom in cardiac_symptoms):
            relevant_data.append(scan)
    
    for dive in data.get("deep_dives", []):
        if any(symptom in str(dive).lower() for symptom in cardiac_symptoms):
            relevant_data.append(dive)
...
# Add similar pattern extraction functions for other specialties...
