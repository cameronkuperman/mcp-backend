...
# Load .env file (place your OPENROUTER_API_KEY in .env at project root)
load_dotenv()
...
async def get_user_model(user_id: str) -> str:
    ...
    return "tngtech/deepseek-r1t-chimera:free"  # Default free model
...
async def call_llm(messages: list, model: Optional[str] = None, user_id: Optional[str] = None, temperature: float = 0.7, max_tokens: int = 2048, top_p: float = 1.0) -> dict:
    ...
    # Fetch model if not provided (and user_id is available)
    if not model and user_id:
        model = await get_user_model(user_id)
    elif not model:
        model = "tngtech/deepseek-r1t-chimera:free"

    # Messages should already be in proper format from build_messages_for_llm
    ...
    async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
        async with session.post(
            ...
            headers={
                ...
                "HTTP-Referer": os.getenv("APP_URL", "http://localhost:3000"),  # Your site for OpenRouter stats
                "X-Title": "Medical Chat API",  # Optional: App name
            ...
            if resp.status != 200:
                ...
                # TODO: Add logging/retry logic here
            ...
            return {
                "content": parsed_content,  # Could be string or dict/list if JSON
                "raw_content": content,     # Always the raw string
...
# Legacy functions for backward compatibility with MCP tools
async def get_chat_history(user_id: str, chat_id: str) -> str:
    ...
    if not messages:
        return None
    # Format as simple string for legacy compatibility
...
async def update_chat_history(user_id: str, chat_id: str, prompt: str, response: str) -> None:
    ...
    # Store the response as assistant message
